...
# ファイル名が日本語だと内部処理で文字化けしてzip path is too longエラーが発生することも
...
# filter (sex == c(0,1))のような書き方だとうまく抽出できない場合あるので上記を使うこと
...
# [Tips] 集計関数n() はsummarise()と組み合わせてデータの行数をカウントします。

# [Tips] df %>% tally(category) を使うとより簡単に件数をカウントできます。
